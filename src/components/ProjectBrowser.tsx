    ...
    <div className="flex flex-col md:flex-row gap-8 font-mono py-16">
      ...
      <div className="w-full md:w-64 flex-shrink-0 max-w-4xl mx-auto w-full px-2 font-mono">
        ...
        <div className="flex flex-col gap-2">
          {allTags.map(tag => (
            <label key={tag} className="flex items-center gap-3 cursor-pointer group hover:opacity-70">
              <div className={`w-4 h-4 border transition-colors duration-200 flex items-center justify-center
                ${selectedTags.includes(tag) ? 'bg-black border-black text-white' : 'border-gray-300 bg-white'}`}
              >
                {/* Checkmark Icon */}
      ...
      <div className="flex-grow max-w-4xl mx-auto w-full px-4 font-mono ">
        ...
        <div className="flex flex-col">
          {filteredProjects.length > 0 ? (
            ...
            <div className="py-12 text-gray-400 italic">No projects match these filters.</div>
          )}
        </div>
      </div>
